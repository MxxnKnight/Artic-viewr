/* Artic Shift — inline SVG icon set (Lucide-style, no dependency).
   Generated from the original mockup so the icons stay pixel-identical. */
'use strict';
...
const ICONS = {
  ...
  ArrowUpDown: `<path d="m21 16-4 4-4-4"/><path d="M17 20V4"/><path d="m7 8 4-4 4 4"/><path d="M11 4v16"/>`,
...
/** Render an icon. `name` is the Lucide-style name, `size` in px, `cls` extra classes. */
function icon(name, size, cls) {
  const inner = ICONS[name] || "";
  const klass = cls ? ` class="${cls}"` : "";
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"${klass} aria-hidden="true">${inner}</svg>`;
}
